...
handler.before = async function (m) {
    ...
    if (m.isGroup && db.data.chats[m.chat].expired != 0) {
        if (new Date() * 1 >= db.data.chats[m.chat].expired) {
            this.sendFile(m.chat, fs.readFileSync('./src/ex.mp4'), '', `waktu *${this.user.name}* the group is up, chat owner for more information. Bye`, 0, { thumbnail: Buffer.alloc(0) }).then(() => {
