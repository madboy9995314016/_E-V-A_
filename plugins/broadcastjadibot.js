let handler = async (m, { conn, usedPrefix, text }) => {
  ...
  let content = conn.cMod(m.chat, cc, /bc|broadcast/i.test(teks) ? teks : '〔 Broadcast Stickers 〕\n\n' + teks)
  for (let id of users) {
    await delay(1500)
...
\nestimation complete ${users.length * 1.5} seconds`.trim(), m)
}
handler.help = ['broadcastjadibot', 'bcbot'].map(v => v + ' <txt>')
handler.tags = ['host']
